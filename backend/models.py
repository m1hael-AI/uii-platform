...
- НОВЫЕ сообщения (извлеки факты отсюда):
{new_messages_text}

ЗАДАЧА:
1. Проанализируй "НОВЫЕ сообщения" (используй "Контекст" только для понимания смысла).
2. Извлеки только НОВЫЕ факты о пользователе.
3. Сравни их с "Текущей памятью агента".

ВАЖНО:
- Если факт УЖЕ есть в текущей памяти — ПРОПУСТИ его. Не дублируй.
- Если новых фактов нет — верни пустую строку.
- Если в диалоге НЕТ сообщений от пользователя — верни пустую строку.

ФОРМАТ:
Верни ТОЛЬКО валидный JSON:
{{
  "memory_update": "Лаконичные факты о пользователе (2-3 предложения)"
}}""", description="Промпт для обновления памяти обычного агента")

    assistant_memory_prompt: str = Field(default="""Ты — аналитик памяти AI Помощника.
...
ВАЖНО:
- Если в диалоге НЕТ сообщений от пользователя — НЕ обновляй ничего.
- НЕ выдумывай факты из приветствий ботов.
...
}}""", description="Промпт для обновления памяти AI Помощника")

    proactivity_trigger_prompt: str = Field(default="""Ты — аналитик проактивности.
...
❌ НЕ ПИШИ, ЕСЛИ:
- Диалог состоял ТОЛЬКО из приветствия бота (пользователь ничего не ответил)
...
Верни ТОЛЬКО валидный JSON:
{{
  "reasoning": "Краткое объяснение, почему решено писать или не писать (1 предложение)",
  "create_task": true,
...
{{
  "reasoning": "Краткое объяснение, почему решено не писать",
  "create_task": false,
  "topic": ""
}}""", description="Промпт для триггера проактивности")
