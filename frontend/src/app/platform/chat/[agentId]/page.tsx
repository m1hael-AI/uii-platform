      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto bg-gray-50/50 space-y-4 md:space-y-6">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && (
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs mr-2 shrink-0 shadow-sm overflow-hidden ${!agentAvatarUrl ? `${agentStyle.bg} ${agentStyle.text}` : "bg-gray-100"}`}>
                {agentAvatarUrl ? (
                  <img src={agentAvatarUrl} alt={agentName} className="w-full h-full object-cover" />
                ) : (
                  agentName[0]
